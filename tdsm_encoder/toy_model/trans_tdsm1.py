...
class CNNnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1=nn.Conv2d(in_channels=4,out_channels=3,kernel_size=5)
...
def loss_fn(model, x, incident_energies, marginal_prob_std, eps=1e-5, device='cuda'):
    '''
    ...
    - Perturbation avoids the need to calculate the trace of Jacobian of model output

    Args:
        model: A PyTorch model instance that represents a time-dependent score-based model
        x: A mini-batch of training data
        incident_energies: A tensor of incident energies
        marginal_prob_std: A function that gives the standard deviation of the perturbation kernel
        eps: A tolerance value for numerical stability
        device: 'cuda' for running on GPUs or 'cpu' for running on CPUs
    '''
    
    # Ensure all tensors are on the specified device
    print(device)
    x = x.to(device)
    ...
    # Inverse mask to ignore when 0-padded hits should be ignored
    ...
    # Tensor of randomized 'time' steps
    random_t = torch.rand(incident_energies.shape[0], device=device) * (1. - eps) + eps
    
    # Noise input multiplied by mask so we don't perturb zero-padded values to have non-sentinel values
    z = torch.normal(0, 1, size=x.shape, device=device)
    z = z.to(device)
    
    # Sample from the standard deviation of noise
    mean_, std_ = marginal_prob_std(x, random_t)
    
    # Add noise to input
    perturbed_x = mean_ + std_[:, None, None] * z #really
    ...
    # Losses across all hits and 4-vectors (normalize by the number of hits)
    losses = torch.mean(losses, dim=(0, 1, 2))
...
def pc_sampler(score_model, marginal_prob_std, diffusion_coeff, sampled_energies, sampled_hits, batch_size=1, snr=0.16, device='cuda',eps=1e-3):
  ...
  padded_hits = len(sampled_energies)
  ...
  for i in range(batch_size):
        init_x_test=torch.normal(0,1,size=(int(sampled_hits[i]),4),device=device)
        pad_hits = 150-int(sampled_hits[i])
        padded_shower = F.pad(input = init_x_test, pad=(0,0,0,pad_hits), mode='constant', value=0.0)
        begin_noise.append(padded_shower)
  #init_x = torch.rand(batch_size, gen_n_hits, 4, device=device)
  #begin_noise = torch.normal(0,1,size=init_x.shape, device=init_x.device)
  #sampled_hits = (torch.tensor(sampled_energies[1])).shape()[0]
  #padded_hits = (torch.tensor(sampled_energies[2])).shape()[0]
    # Pad initial input in same way input to model looked during training
  #pad_hits = padded_hits-sampled_hits
  #padded_shower = F.pad(input = init_x, pad=(0,0,0,pad_hits), mode='constant', value=0.0)
  ...
  #padded_hits = torch.tensor(sampled_energies[2])
  #pad_hits = padded_hits-sampled_hits
  #padded_shower = F.pad(input = init_x, pad=(0,0,0,0), mode='constant', value=-20)
    
  #mean_, std_ = marginal_prob_std(padded_shower, t)
  #std_.to(device)
  #x = padded_shower#*std_[:,None,None]
  #print("2",init_x)
  #print("std:",std_[:,None,None])
  #time_steps = np.linspace(1., eps, num_steps)
  #step_size = time_steps[0] - time_steps[1]
  ...
  with torch.no_grad():
    ...
    for time_step in tqdm(time_steps, desc="Processing"):
      ...
      #print("diff",diff)
      #x_mean = x + (diff**2)[:, None, None] * step_size * score_model(x, batch_time_step, sampled_energies, mask=padding_mask)
      ...
      flattened_scores = flattened_scores.to(device)
      grad_norm = grad_norm.to(device)
      flattened_noise = flattened_noise.to(device)
      noise_norm = noise_norm.to(device)
      langevin_step_size = langevin_step_size.to(device)
      x_mean = x_mean.to(device) 
      
      #x = x_mean + torch.sqrt(diff**2 * step_size)[:, None, None] * z
      if (time_step == (num_steps-2*eps)):
